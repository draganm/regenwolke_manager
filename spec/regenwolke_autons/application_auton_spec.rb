module RegenwolkeAutons
  ...
  describe ApplicationAuton do
    ...
    describe '#set_environment' do
      ...
      it 'should merge existing and new environment' do
        subject.set_environment('c' => 'd')
        expect(subject.environment).to eq({"a"=>"b", "c"=>"d"})
      end
      ...
      context 'when there is an existing deployment' do
        ...
        it 'should schedule #set_environment on the deployment' do
          subject.set_environment('c' => 'd')
          expect(context).to have_received(:schedule_step_on_auton).with('deployment:app1:some_sha', :set_environment, [{"a"=>"b", "c"=>"d"}])
        end
      end
    ...
    describe '#deploy' do
      ...
      it 'should start a new deployment process' do
        ...
        expect(context).to receive(:schedule_step_on_auton).with('deployment:app1:some_sha', :start, ['app1','some_sha', {'a' => 'b'}])
